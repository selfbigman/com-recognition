...
from ultralytics.nn.extra_modules.MyPruner import DiverseBranchBlockPruner, LayerNormPruner, RepConvPruner, DyHeadBlockPruner

# 没有购买yolov8项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, FocalModulation
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, Faster_Block):
            ignored_layers.append(m.mlp[-1])
    ignored_layers.append(model.model[2].cv2)
    ignored_layers.append(model.model[4].cv2)
    ignored_layers.append(model.model[6].cv2)
    ignored_layers.append(model.model[8].cv2)
    ignored_layers.append(model.model[15].cv2)
    ignored_layers.append(model.model[19].cv2)
    ignored_layers.append(model.model[24].cv2)
    ignored_layers.append(model.model[27].cv2)
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, TopBasicLayer):
            ignored_layers.append(m)
        if isinstance(m, FocalModulation):
            ignored_layers.append(m)
